...
def simulate(agent, i, episodes = 1000, learning_rate=0.1):
    ...
    while not done:        
        ...
        # Pick a greedy action
            ...
            a = agent.Qtable[state["time_month"], :]
            action = np.argmax(a)
        ...
        a = agent.Qtable[next_state["time_month"]]
        Q_target = (reward + agent.discount_rate*np.max(a))
        
        # Calculate the Temporal difference error (delta)
        delta = learning_rate * (Q_target - agent.Qtable[state["time_month"], action])
        
        # Update the Q-value
        if phase_nr == 0:
            agent.Qtable[state["time_month"], action] = agent.Qtable[state["time_month"], action] + delta
...
if __name__ == "__main__":
    ...
    if not eval:
        ...
        df.to_csv('runs/q_agent_month/eval.csv')