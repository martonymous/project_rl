from environment_lstm import DamWorldEnv
from utils import *
import pandas as pd
import numpy as np
...
def simulate(agent, i, episodes = 1000, learning_rate=0.1):
    ...
    while not done:        
        ...
        # Update the Q-value
        agent.Qtable[
            state["time_hour"], state["time_weekend"], state["time_month"], state["lstm"], state["electricity_cost"], state["water_level"], action
        ] = agent.Qtable[
            state["time_hour"], state["time_weekend"], state["time_month"], state["lstm"], state["electricity_cost"], state["water_level"], action
        ] + delta
