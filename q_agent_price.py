...
def simulate(agent, i, episodes = 1000, learning_rate=0.1):
    ...
    while not done:        
        ...
        # Pick a greedy action
            ...
            a = agent.Qtable[state["electricity_cost"], :]
            action = np.argmax(a)
        ...
        a = agent.Qtable[next_state["electricity_cost"]]
        Q_target = (reward + agent.discount_rate*np.max(a))
        
        # Calculate the Temporal difference error (delta)
        delta = learning_rate * (Q_target - agent.Qtable[state["electricity_cost"], action])
        
        # Update the Q-value
        agent.Qtable[state["electricity_cost"], action] = agent.Qtable[state["electricity_cost"], action] + delta
...
if __name__ == "__main__":
    ...
    if not eval:
        ...
        df.to_csv('runs/q_agent_price/eval.csv')